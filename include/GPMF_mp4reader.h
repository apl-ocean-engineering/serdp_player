...
#define BYTESWAP64(a)                                                          \
  (((a & 0xff) << 56) | ((a & 0xff00) << 40) | ((a & 0xff0000) << 24) |        \
   ((a & 0xff000000) << 8) | ((a >> 56) & 0xff) | ((a >> 40) & 0xff00) |       \
   ((a >> 24) & 0xff0000) | ((a >> 8) & 0xff000000))
#define BYTESWAP32(a)                                                          \
  (((a & 0xff) << 24) | ((a & 0xff00) << 8) | ((a >> 8) & 0xff00) |            \
   ((a >> 24) & 0xff))
#define BYTESWAP16(a) ((((a) >> 8) & 0xff) | (((a) << 8) & 0xff00))
#define NOSWAP8(a) (a)

#define MOV_GPMF_TRAK_TYPE                                                     \
  MAKEID('m', 'e', 't', 'a') // track is the type for metadata
...
#define AVI_VIDS_TRAK_TYPE                                                     \
  MAKEID('v', 'i', 'd', 's') // track is the type for video
#define AVI_CFHD_SUBTYPE MAKEID('c', 'f', 'h', 'd') // subtype is CineForm HD

#define NESTSIZE(x)                                                            \
  {                                                                            \
    int i = nest;                                                              \
    while (i > 0 && nestsize[i] > 0) {                                         \
      nestsize[i] -= x;                                                        \
      if (nestsize[i] >= 0 && nestsize[i] <= 8) {                              \
        nestsize[i] = 0;                                                       \
        nest--;                                                                \
      }                                                                        \
      i--;                                                                     \
...
#define VALID_FOURCC(a)                                                        \
  (((((a >> 24) & 0xff) >= 'a' && ((a >> 24) & 0xff) <= 'z') ||                \
    (((a >> 24) & 0xff) >= 'A' && ((a >> 24) & 0xff) <= 'Z') ||                \
    (((a >> 24) & 0xff) >= '0' && ((a >> 24) & 0xff) <= '9') ||                \
    (((a >> 24) & 0xff) == ' ')) &&                                            \
   ((((a >> 16) & 0xff) >= 'a' && ((a >> 24) & 0xff) <= 'z') ||                \
    (((a >> 16) & 0xff) >= 'A' && ((a >> 16) & 0xff) <= 'Z') ||                \
    (((a >> 16) & 0xff) >= '0' && ((a >> 16) & 0xff) <= '9') ||                \
    (((a >> 16) & 0xff) == ' ')) &&                                            \
   ((((a >> 8) & 0xff) >= 'a' && ((a >> 24) & 0xff) <= 'z') ||                 \
    (((a >> 8) & 0xff) >= 'A' && ((a >> 8) & 0xff) <= 'Z') ||                  \
    (((a >> 8) & 0xff) >= '0' && ((a >> 8) & 0xff) <= '9') ||                  \
    (((a >> 8) & 0xff) == ' ')) &&                                             \
   ((((a >> 0) & 0xff) >= 'a' && ((a >> 24) & 0xff) <= 'z') ||                 \
    (((a >> 0) & 0xff) >= 'A' && ((a >> 0) & 0xff) <= 'Z') ||                  \
    (((a >> 0) & 0xff) >= '0' && ((a >> 0) & 0xff) <= '9') ||                  \
    (((a >> 0) & 0xff) == ' ')))
...
uint32_t GetPayloadTime(size_t handle, uint32_t index, float *in,
                        float *out); // MP4 timestamps for the payload
...
double GetGPMFSampleRateAndTimes(
    size_t handle, GPMF_stream *gs, double lastrate, uint32_t index, double *in,
    double *out); // Jitter corrected sample(s) time, if lastrate is unknown,
                  // send 0.0 and it will be computed
